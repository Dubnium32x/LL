...
void Drip_Init(void) {
    ...
    if (!s_stainBuf)
        s_stainBuf = pd->graphics->newBitmap(SCR_W, SCR_H, kColorBlack);
    Drip_Clear();
}
...
void Drip_Update(f32 deltaTime) {
    for (i32 i = 0; i < DRIP_MAX; i++) {
        ...
        if (d->tipY >= d->stopY) {
            d->tipY = d->stopY;
            d->stopped = true;
            // Bake the finished drip into the stain buffer as white-on-black — Drip_Draw
            // composites this with kDrawModeXOR, where white source pixels invert whatever's
            // on screen (so the stain reads white against the black room but dark against any
            // solid white world geometry, e.g. a ceiling tile) and black source pixels are a
            // no-op (transparent).
            if (s_stainBuf) {
                pd->graphics->pushContext(s_stainBuf);
                ...
                if (h > 0) pd->graphics->fillRect(x, y, d->width, h, kColorWhite);
                // blob at tip
                pd->graphics->fillEllipse(x - 1, (i32)d->tipY - 1, d->width + 2, d->width + 2, 0, 360, kColorWhite);
                pd->graphics->popContext();
                d->active = false;
            }
        }
...
void Drip_Draw(void) {
    ...
    // Draw stain buffer (dried drips), inverted onto whatever's underneath so it stays visible
    // against both the black background and any solid white world geometry.
    if (s_stainBuf) {
    ...
    // Draw active (falling) drips — kColorXOR inverts in place, same reasoning as above.
    for (i32 i = 0; i < DRIP_MAX; i++) {
        Drip* d = &s_drips[i];
        if (!d->active || d->stopped) continue;
        i32 x = (i32)d->x - d->width / 2;
        ...
        if (h > 0) pd->graphics->fillRect(x, y, d->width, h, kColorXOR);
        // teardrop blob at tip
        pd->graphics->fillEllipse(x - 1, (i32)d->tipY - 1, d->width + 2, d->width + 2, 0, 360, kColorXOR);
    }
}
